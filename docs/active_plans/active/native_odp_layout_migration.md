...
Status: implementation in progress. Statements labeled **target** are required future behavior until
their named work package and acceptance evidence are complete; current behavior is recorded only in
...
- `slide_lib/_layout_registry.py` owns the declarative 18-`LayoutContract` catalog only.
  `slide_lib/_layout_measurement.py` owns pure measurement, preflight, pagination, and local-heading
  calculation. `slide_lib/_layout_builders.py` owns construction of planned objects from the already
...
- Touch points: `slide_lib/layout_engine.py`, `slide_lib/layout_primitives.py`,
  `slide_lib/layout_content.py`, and new private `_layout_registry.py`, `_layout_measurement.py`, and
  `_layout_builders.py`; extract and migrate all format-neutral authority to the layered plan and
...
- Acceptance criteria: `_layout_registry.py` is at most 450 physical lines, contains the declarative
  18 `LayoutContract` catalog, and has no callback or project-module import.
  `_layout_measurement.py` is at most 650 physical lines and owns only pure
  measurement/preflight/pagination/local-heading work. `_layout_builders.py` is at most 700 physical
...
- Implementation evidence: accepted with 171 focused tests. The compiler produces all 18 layouts
  ...
  `layout_engine.py` 290, `_layout_registry.py` 77, `_layout_measurement.py` 634, and
  `_layout_builders.py` 669. The implementation measures committed face profiles at exact point
...
- [ ] WP-E1 and WP-E2 publish native LibreOffice contract evidence.
- [ ] WP-L1 freezes the physical layout model.
- [ ] WP-T1 updates and verifies the OTP 36/28 contract.
- [ ] WP-T2 verifies committed font assets, immutable profiles, exact measurement, and runtime-drift evidence before compiler capacity acceptance.
- [x] WP-L2 compiles all 18 layouts without format imports (171 focused tests; independently
  accepted 2026-09-08).
- [ ] WP-O1 through WP-O3 produce feature-complete static native ODP.
- [ ] WP-P1 preserves independent PPTX output.
- [ ] WP-A1 passes structural and state-harness reveal acceptance.
- [ ] WP-I1 removes the PPTX-to-ODP path and obsolete modules.
- [ ] WP-V1 passes focused and full permanent pytest.
- [ ] WP-V2 passes the native-layout E2E and `lect02a` autonomous acceptance.
- [ ] WP-D1 synchronizes all durable documentation and changelog entries.
- [ ] WP-D2 resolves every blocker and high-risk audit finding.
- [ ] The integrator archives this completed plan with final evidence.
...
- Completion notes: link the final acceptance report and independent review from the archived plan.
